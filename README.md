# Hi, I'm Kawoh Ezekiel
### Data & Machine Learning Engineer

I have several years of hands-on experience transforming complex datasets into actionable insights, coupled with a masters degree in machine learning that gave me the opportunity to advance my knowledge domain. I also have strong background in data visualization, predictive analytics, database management, and business intelligence. Proficient with modern analytics tools including Python, SQL, Power BI, Excel, Databricks, Google Analytics, AWS and Azure services. Proven ability to improve decision-making, optimize business processes, and support cross-functional teams through data-driven solutions. 

---

## 🛠️ Technical Skills

* **Data & Analytics:**  Python, SQL, R, Snowflake, Databricks, Excel, Power BI, Tableau, Google Analytics & GA4, MLOps Basics, A/B Testing. 
* **Machine Learning:**  Gradient Boosting, Regression, Model Deployment, Feature Engineering, Clustering, NLP, Time Series Analysis, TensorFlow, PySpark, PyTorch. 
* **Database & Cloud:**  MySQL, PostgreSQL, AWS, Azure DevOps, GitHub/GitLab.
* **Soft Skills:**  Communication, Team Collaboration, Critical Thinking, Leadership, 
Problem-Solving, Time Management.
* **Domain Expertise:**  Web Development, Digital Marketing Analytics, Dashboard 
Design, Funnel Analysis.

---

## 🚀 Featured Projects

### 1. [Project Title 1: e.g., Predictive Machine Learning Model]
* **Business Problem:** Briefly explain what challenge this project solved.
* **Tools Used:** Python (Scikit-Learn, Pandas), Databricks
* **What I Did:** Bullet points detailing your analysis, modeling, or pipeline engineering choices.
* **Impact:** The tangible result (e.g., "Improved prediction accuracy by 15%").
* **Links:** [🔗 Live Interactive Dashboard](#) | [💻 Code Repository](link-to-your-other-project-repo)

### 2. [Project Title 2: e.g., Enterprise BI Dashboard]
* **Business Problem:** What data was hidden that you brought to light?
* **Tools Used:** SQL, Power BI
* **What I Did:** Mention your ETL process, data modeling (star schema), and DAX measures.
* **Links:** [🔗 View Interactive Report](#) | [💻 SQL Queries](link-to-repo)

---

## 📫 Let's Connect!
* [LinkedIn](www.linkedin.com/in/kawohezekiel)
* [Email](mailto:your.ezekielkawoh@gmail.com)
